...
class Parameter extends Element, @param, LocalScopeVariable {
  ...
  /**
   * Gets an argument for this parameter in any call to the callable that declares it.
   */
  Expr getAnArgument() {
    not this.isVarargs() and
    exists(Call call, int i |
      i = this.getPosition() and
      result = call.getArgument(i) and
      call.getCallee().getSourceDeclaration().getAParameter() = this
    )
  }

  /** Gets an assigned value. */
  Expr getAnAssignedValue() { none() }
