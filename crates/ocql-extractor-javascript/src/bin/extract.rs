//! Extract JavaScript/TypeScript source files into a database and dump as CSV.
//!
//! Usage:
//!   extract-javascript <source-dir> <output-dir> [--typescript]
//!
//! Detects project structure and extracts from source roots.
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: extract-javascript <source-dir> <output-dir> [--typescript]");
        std::process::exit(1);
    }

    let source_dir = Path::new(&args[1]);
    let output_dir = Path::new(&args[2]);
    let use_typescript = args.iter().any(|a| a == "--typescript");
    ...
    let results = if use_typescript {
        let extractor = TypeScriptExtractor::new();
        extractor.extract_directory(&mut db, source_dir)
    } else {
        // Extract both JS and TS files
        let js_extractor = JavaScriptExtractor::new();
        let ts_extractor = TypeScriptExtractor::new();
        let mut results = js_extractor.extract_directory(&mut db, source_dir);
        results.extend(ts_extractor.extract_directory(&mut db, source_dir));
        results
    };
